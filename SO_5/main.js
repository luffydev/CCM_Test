...
const WS_COMMAND_LINE = {
    'refresh' : {desc : 'refresh current browser\'s page (ex : refresh )'},
    'alert'   : {desc : '{msg} show an alert in browser with {msg} as content (ex : alert hello world )'},
    'confirm' : {desc : '{question} show a confirm alert with {question} as content (ex : confirm are you sure ?)'},
    'goto'    : {desc : '{url} redirect browser to {url} (ex : goto https://google.fr)'},
    'clear'   : {desc : 'clear current server console window'} 
}
...
// Here we put our WebSocket Server for sending command line to our client
const lWSServer = new websocket.Server({server : lServer });


lWSServer.on('connection', (pSocket) => {
    
    const lLineReader = readline.createInterface({
        input: process.stdin,
    ...
    // We log when we receive any message from client's socket
    pSocket.on('message', (pData) => {
        logger.info(`Recv MSG from client : ${pData}`, 'WS');
    });


    // When client close websocket
    pSocket.on('close', () => {
        logger.info("Client disconnected from websocket !", 'WS');

        lLineReader.close();
    });

    pSocket.send("CMD Line is now connected !");
    logger.success("Welcome to Websocket Command line !", 'WS');
    logger.info("Type any command to send to client ! ( type 'help' to see list of commands ) ");

    lLineReader.setPrompt("> ");
    lLineReader.prompt();

    // fired each time the input has a new line   
    lLineReader.on('line', function(pCMD){     

        // if we type clear command, we clear console
        if(pCMD === 'clear')
        {
        ...
        // if we type help command we show commands list with desc 
        if(pCMD === 'help'){            
            logger.write("list of all command :");

            for(const [lKey, lValue] of Object.entries(WS_COMMAND_LINE)){
                logger.write(`${lKey}: ${lValue.desc}`);
            }
        ...
        // if it's not 'help' command we send command to our client
        pSocket.send(pCMD);        

        logger.info(`Sending command ${pCMD} to client's socket`, 'WS');
